...
def on_worker_init(sender=None, **kwargs):
    ...
    if worker_type == "fast":
        # Явно инициализируем FastModel с корректными путями
        from worker.ml.fast_model import init_fast_model
        import os
        tfidf_path = os.environ.get("FAST_TFIDF_PATH", "/app/ml/models/tfidf.pkl")
        model_path = os.environ.get("FAST_MODEL_PATH", "/app/ml/models/catboost_model.cbm")
        # Для docker-compose путь внутри контейнера: /app/worker/ml/models/...
        if not os.path.exists(tfidf_path):
            tfidf_path = "/app/worker/ml/models/tfidf.pkl"
        if not os.path.exists(model_path):
            model_path = "/app/worker/ml/models/catboost_model.cbm"
