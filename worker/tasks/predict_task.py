...
# Синхронный движок для Celery (asyncpg не работает в sync контексте)
...
def run_prediction(
    ...
    try:
        ...
        # 2. Определить model_key по тарифу
        ...
        # 5. Обновить задачу как COMPLETED
        ...
        # 6. Если low_confidence → HITL-очередь
        if low_conf:
            db.add(HITLTask(
        ...
        # Попытка рефанда и обновление статуса
        try:
