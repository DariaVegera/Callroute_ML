...
def register(email: str, password: str, full_name: str, referral_code: str = "") -> tuple[bool, str]:
    try:
        ...
        detail = r.json().get("detail", "Неизвестная ошибка")
...
if not st.session_state.token:
    ...
    with col1:
        ...
        if mode == "Войти":
            ...
            reg_name = st.text_input("Имя (необязательно)", key="reg_name")
            reg_password = st.text_input("Пароль (мин. 8 символов)", type="password", key="reg_password")
            reg_password2 = st.text_input("Повторите пароль", type="password", key="reg_password2")
            reg_ref = st.text_input("Реферальный код (необязательно)", key="reg_ref")
            ...
            if st.button("Зарегистрироваться", use_container_width=True):
                if not reg_email or not reg_password:
                    ...
                    ok, msg = register(reg_email, reg_password, reg_name, reg_ref)
                    if ok:
                        st.success(msg)
                        st.info("Теперь выберите «Войти» и введите свои данные.")
                    else:
                        st.error(msg)
